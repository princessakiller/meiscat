  ...
  function loop(){
    ...
    if(isBeat){
      ...
      if(queue.length){ const [k,v]=queue.shift(); appendKV(k,v); }
        ...
        // subtle pulse on last item
        const last=els.stack.lastElementChild;
        if(last) last.animate([{transform:'scale(1)'},{transform:'scale(1.05)'},{transform:'scale(1)'}],{duration:160,easing:'ease-out'});
      }
    }
  }

  // Fallback interval if music is silent / no beats
  setInterval(()=>{
